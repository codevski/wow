...
#
# Usage:
#   scripts/restore.sh <archive>          # restore DB schemas + configs
#   scripts/restore.sh --latest           # pick newest wow-full-*.tar.zst
#   scripts/restore.sh --no-configs ...   # skip configs/ overwrite
#
# DESTRUCTIVE: drops + recreates each schema from the dump.
# Stops worldserver and authserver first; restarts at the end.
...
[[ -n "$ARCHIVE" ]] || die "provide an archive path or --latest"

if [[ "$ARCHIVE" == "LATEST" ]]; then
    ARCHIVE="$(ls -1t "$WOW_ROOT/backups/wow-full-"*.tar.zst 2>/dev/null | head -1)"
...
for sql in *.sql; do
    ...
    sudo podman exec -i ac-database mysql -uroot -p"$DBPASS" "$schema" < "$sql"
done
