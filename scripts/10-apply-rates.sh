#!/usr/bin/env bash
# Step 11 (optional) — apply a generous rate preset to worldserver.conf.
# x5 XP / x3 talents / boosted drops. Restart worldserver after running.
#
...
[[ -f "$CONF_PATH" ]] || die "$CONF_PATH not found — run 05-populate-configs.sh first"

log "Backing up $CONF_PATH ..."
cp -n "$CONF_PATH" "${CONF_PATH}.bak.$(date +%s)" || true
...
log "Restarting worldserver to pick up new rates..."
sudo podman ps --format '{{.Names}}' | grep -q '^ac-worldserver$' \
    && sudo podman restart ac-worldserver || true
